...
\begin{figure}
\centering
\includegraphics[width=0.8\textwidth]{Bilder/BQ_Tesla_transmitter.JPG}
...
\begin{figure}
\centering
\includegraphics[width=0.8\textwidth]{Bilder/BQ_Tesla_receiver.JPG}
...
\begin{figure}
\centering
\includegraphics[width=1\textwidth]{Bilder/self_receiver.JPG}
...
\begin{figure}
\centering
\includegraphics[width=1\textwidth]{Bilder/CC2541.JPG}
...
\begin{figure}
\centering
\includegraphics[width=1\textwidth]{Bilder/breakout.JPG}
\caption{Modifizierte ez430-Chronos Uhr mit Piezo-Interrupt Taster und Erweiterungsstiftleisten}
Quelle: Stephan Jobstmann
\label{pic.breakout}
\end{figure}
\end{itemize}
...
\begin{figure}
\centering
\includegraphics[width=1\textwidth]{Bilder/CC430.JPG}
...
\begin{figure}
\centering
\includegraphics[width=1\textwidth]{Bilder/ad1232.JPG}
\caption{AD1232 A/D-Wandler Modul mit Drucksensor auf Keramik-Stahl Verbundmaterial}
Quelle: Stephan Jobstmann
\label{pic.ad1232}
\end{figure}
\begin{itemize}
...
Zur Frequenzgenerierung wurde vorerst als Zielbaustein der CC2541 aufgrund des bestehenden Hardware-Aufbaus (vergleiche Kapitel \vref{CC2541}) in den Vordergrund gerückt. Zum Erstellen wurde die Entwicklungsumgebung IAR Embedded Workbench IDE for 8051\footnote{\url{http://www.iar.com/en/Products/IAR-Embedded-Workbench/8051/}} verwendet. Zuerst wurden die grundlegenden Strukturen der Clock-Generierung, der Timer-Funktion und die der Port-Adressierung mithilfe des User's Guide \citep{CC2541_full} ausreichend eruiert. Um den Code auch für Dritte möglichst lesbar zu halten, wurden statt einfachen hexadezimalen Werten zur Registerkonfiguration lesbare Akronyme in einer separaten Header-Datei erstellt. Diese findet man ab Seite \pageref{CC2541_porting_defines}. Der ganze Code wurde wiederum mit Kommentaren versehen um einen schnellen Überblick über dessen Funktion wiederzugeben. Auf Seite \pageref{CC2541_main} findet man die Hauptroutine. Von Zeile \texttt{6} bis \texttt{8} wird als Taktquelle für den Prozessor der externe Quarz eingestellt. Die Port-Konfiguration wurde in der Passage von \texttt{10} bis \texttt{13} modifiziert. Dabei wurde die zweite alternative Portbelegung für den Timerausgang vorgenommen. Timer 1 wird in Zeile \texttt{15} mit \texttt{MODULU} zum einfachen Hochzählen bis zu einem festgelegten Wert (Zeile \texttt{22} bis \texttt{24}) konfiguriert. Mit den Passagen \texttt{22}, \texttt{26} bis \texttt{28} wird ein Puls-Breiten-moduliertes\footnote{PWM: Puls Width Modulation} Signal mit 50\% Taktverhältnis erzeugt. 
\newpage
